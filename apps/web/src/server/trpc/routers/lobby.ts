...
import { getLobbyView, leaveLobby, setReady } from "../../lobby";
...
export const lobbyRouter = createTRPCRouter({
  ...
  /** Leave the lobby before the call starts (counts as a no-show; re-queues the other). */
  leave: protectedProcedure.input(sessionIdInputSchema).mutation(async ({ ctx, input }) => {
    const user = await ensureDbUser(ctx.clerkAuth.userId);
    return leaveLobby(ctx.prisma, input.sessionId, user.id);
  }),
});
